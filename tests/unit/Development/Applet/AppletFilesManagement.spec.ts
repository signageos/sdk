	...
	describe('listAppletFiles', function () {
		...
		it('should list all files not in .sosignore', async function () {
			...
			should(filePaths).eql([
				...
				'dir-1/file-2',
				'dir-1/dir-2/file-3',
			].map((filePath) => path.join(applet2Dirname, filePath)));
		});
		...
		it('should list all files not in .gitignore', async function () {
			...
			should(filePaths).eql([
				...
				'dir-1/file-2',
				'dir-1/dir-2/file-3',
			].map((filePath) => path.join(applet3Dirname, filePath)));
		});
		...
		it('should list all files not in .npmignore', async function () {
			...
			should(filePaths).eql([
				...
				'dir-1/file-2',
				'dir-1/dir-2/file-3',
			].map((filePath) => path.join(applet4Dirname, filePath)));
		});
	});
});
