...
describe('e2e.RestAPI - Runner Version Platform', () => {
	...
	after(async () => {
		...
		try {
			...
			for (const platform of platforms) {
				try {
					...
					console.warn(`Failed to delete platform ${platform.platform} during cleanup:`, error);
				}
			}
		} catch (error) {
			console.warn('Failed to list platforms during cleanup:', error);
		}
		...
		try {
			...
			console.warn('Failed to delete runner version during cleanup:', error);
		}

		try {
			await api.runner.delete(runnerUid);
		} catch (error) {
			console.warn('Failed to delete runner during cleanup:', error);
		}
	});
