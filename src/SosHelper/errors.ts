...
export class AccountAuthMissingError extends BaseError {
	constructor() {
		super(
			`Account auth is missing.`,
			[`Please install cli tool globally using "npm i @signageos/cli -g" and do "sos login" first.`],
		);
...
export class AppletNotFoundError extends BaseError {
	constructor(
		...
		super(
			`Applet "${appletName}" is not found.`,
...
export class MultipleAppletFoundError extends BaseError {
	constructor(
		...
		super(
			`Multiple applets with name "${appletName}" are found: ${appletUids.join(', ')}.`,
			[
